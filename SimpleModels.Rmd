...
## 3. Linear Model with One Predictor

Now that we have selected the best variable to use for our models, we can fit a generalized linear model on the first predictor, "Government should reduce differences in income levels". 
```{r cache=TRUE}
# Try horseshoe prior
t_prior <- student_t(df = 5, location = 0.5, scale = 0.1)
...
new_data <- data.frame(
  ...
  gincdif = g_steps)
pred_post <- posterior_predict(fit1, newdata = new_data)
dim(pred_post)
