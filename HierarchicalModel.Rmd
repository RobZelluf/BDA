...
```{r linear model for varsel, cache=TRUE, results=FALSE}
...
```{r seperate models, cache=TRUE, results=FALSE}
...
```{r hierarchical model, cache=TRUE, results=FALSE}
fit_hier_2 <- stan_lmer(lrscale ~ 1 + gincdif + impcntr + dfincac + rlgblg + sbsrnen + (1 + gincdif + impcntr + dfincac + rlgblg + sbsrnen | country), data=sample_data_best_5)
```
...
Following are the slope and intercept parameters for each country in the hierarchical model. Note that they generally agree at least about the direction of the correlation, except in a few interesting cases.
```{r}
coefficients <- coef(fit_hier_2)[1]
```

### Effective Sample Size
As we can see in the summary for each of the three models, the effective sample size for each parameter is at least 3000, which is plenty!
...
```{r one predictor model, cache=TRUE, results=FALSE} 
# Try horseshoe prior
t_prior <- student_t(df = 5, location = 0.5, scale = 0.1)
...
```{r posterior prediction plot pooled }
...
```{r posterior prediction plot pooled 2}
...
```{r posterior prediction plot partially pooled}
g_rng <- range(sample_data_best_5$gincdif) 
g_steps <- seq(g_rng[1], g_rng[2], length.out = 80)
new_data <- data.frame(
  observation = seq_along(g_steps), 
  country = "FI"
  gincdif = g_steps)
pred_post <- posterior_predict(fit_hier_2, newdata = new_data)
dim(pred_post)
...
Correlation plots: \url{https://avehtari.github.io/modelselection/diabetes.html}
Posterior predictive plotting: \url{https://www.tjmahr.com/visualizing-uncertainty-rstanarm/}
Variable selection: \url{https://mc-stan.org/projpred/articles/quickstart.html}
Partial pooling: \url{http://mc-stan.org/rstanarm/articles/pooling.html}
  and \url{https://mc-stan.org/users/documentation/case-studies/tutorial_rstanarm.html}
